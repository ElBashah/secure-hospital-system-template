...
export const getPatientInformation = async (patient) => {

    try{
        // const _patient = (await db.doc(`patients/${patient.pateint_ref}`).get()).data()
        const _user = (await db.doc(`users/${patient.user_ref}`).get()).data()
        return _user

    }catch(e){
        logError("getPatientInformation", e)
        return null
    }
}
